...
import { modelQuizQuestions, userStorageKey } from '../data/quizData';
...
function DetailedQuiz() {
  ...
  const handleNext = () => {
    ...
    if (currentQuestionIndex < modelQuizQuestions.length - 1) {
      ...
      const modelAnswers = {
        Math: parseInt(finalAnswers.math) || 0,
        Biology: parseInt(finalAnswers.biology) || 0,
        Computer: parseInt(finalAnswers.computer) || 0,
        Communication: parseInt(finalAnswers.communication) || 0,
        Marks: parseInt(finalAnswers.marks) || 0,
        Background: finalAnswers.background || '',
        Leadership: parseInt(finalAnswers.leadership) || 0,
      };

      localStorage.setItem(userStorageKey('modelQuizAnswers', currentUser?.id), JSON.stringify(modelAnswers));
      // also persist detailed/model answers to Supabase if signed-in
      if (currentUser?.id) {
  ...
  const handleSkip = () => {
    ...
    if (window.confirm('Skipping will use default values. Continue?')) {
      localStorage.setItem(userStorageKey('modelQuizAnswers', currentUser?.id), JSON.stringify({ Math: 0, Biology: 0, Computer: 0, Communication: 0, Marks: 0, Background: '', Leadership: 0 }));
      navigate('/result');
    }
  };
